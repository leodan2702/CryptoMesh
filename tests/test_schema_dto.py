...
from cryptomesh.dtos import SchemaDTO,ParameterSpecDTO
...
"""
    ...
    assert result.init == [ParameterSpecDTO(name="source_bucket", type="str", default=None), ParameterSpecDTO(name="source_key", type="str", default=None)]
    assert "run" in result.methods
    assert result.methods["run"] == [ParameterSpecDTO(name="multiplier", type="int", default=None)]
    assert "process_async" in result.methods
    process_sync = result.methods["process_async"]
    assert process_sync == [ParameterSpecDTO(name="item_id", type="Any", default=None)]
...
"""
    ...
    # Assert
    assert result.class_name == expected_class_name
    assert result.init == [ParameterSpecDTO(name="name", type="Any", default=None)]
...
"""
    ...
    assert result.methods["do_work"] == [ParameterSpecDTO(name="task_id", type="Any", default=None)]